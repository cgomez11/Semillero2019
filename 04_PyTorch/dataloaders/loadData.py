...
train_dataset = myDataset(partition['train'], labels, data_path + 'train/', transform=transforms.Compose([transforms.ToTensor()]))
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

val_dataset = myDataset(partition['val'], labels, data_path + 'val/', transform=transforms.Compose([transforms.ToTensor()]))
